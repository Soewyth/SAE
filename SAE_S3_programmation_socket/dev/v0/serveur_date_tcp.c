...
int main(int argc, char *argv[]){
	...
	while(1){
		...
		if (messageRecu[0]=='D' || messageRecu[0]=='d') {
			lire_date(messageEnvoye);
		} else if (messageRecu[0]=='H' || messageRecu[0]=='h') {
			lire_heure(messageEnvoye);
		} else {
			snprintf(messageEnvoye, LG_MESSAGE, "Commande inconnue !\n");
		}

		// On envoie la réponse au client
